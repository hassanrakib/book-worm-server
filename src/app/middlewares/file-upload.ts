...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(
      null,
      `${file.originalname}-${file.fieldname}-${uniqueSuffix}${path.extname(
        file.originalname
      )}`
    );
  },
  ...
  if (allowedTypes.includes(file.mimetype)) {
    ...
    cb(
      new AppError(
        ...
        "Invalid file type. Only Webp, JPEG & PNG are allowed"
      )
    );
  }
};
...
const fileUpload = () => {
  return catchAsync(async (req, res, next) => {
    ...
    try {
      ...
      if (req.body?.data) {
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
        req.body = JSON.parse(req.body.data as string) as Record<
          string,
          unknown
        >;
      }
